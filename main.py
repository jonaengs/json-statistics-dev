...
from utils import print_memory_use
    ...
    def record_path_stats(stats, parent_path, key, val) -> str:
        match STATS_TYPE:
            ...
            case StatType.HYPERLOG:
                ...
                return key_str
            case _:
                assert False, "statistics type not supported"
...
def make_statistics(collection) -> list[dict, dict]:
    ...
    print("num_pruned", num_pruned, f"({len(base_stats)} unique paths total)")
...
if __name__ == '__main__':
    print(f"using {StatType(STATS_TYPE).name} StatType")
    file_name = sys.argv[1] if len(sys.argv) > 1 else "test"

    data_path = f"data/recsys/{file_name}.json"
    stats_path = f"stats/{file_name}.json"
    ...
    # print(compute_cardinality(data_path, lambda d: d["entities"]["urls"][0]["url"]))
    # print(get_stats_data(stats_path, lambda d: d["entities_object.urls_array.0_object.url_str"])["count"])
    # print()
    
    # print(compute_ndv(data_path, lambda d: d["entities"]["urls"][0]["url"]))
    # print(get_stats_data(stats_path, lambda d: d["entities_object.urls_array.0_object.url_str"])["ndv"])
    # print()

    # # "entities_object.hashtags_array.0_object.text_str"
    # print(compute_ndv(data_path, lambda d: d["entities"]["hashtags"][0]["text"]))
    # print(get_stats_data(stats_path, lambda d: d["entities_object.hashtags_array.0_object.text_str"])["ndv"])
    # print()